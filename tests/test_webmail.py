...
def test_get_discussion_mail_list(api):
    ...
    assert_response_succeeded(response)
    assert (
        response["DiscussionMailList"]["DiscussionMail"][0]["DiscussionId"] == 113163877
    )
    assert (
        response["DiscussionMailList"]["DiscussionMail"][0]["OperationStatus"]
        == "DiscussionClosed"
    )
    assert (
        response["DiscussionMailList"]["DiscussionMail"][0]["MailAddress"] is not None
    )
...
def test_get_discussion_mail_list_with_nonexistent_discussions(api):
    ...
    assert response["DiscussionMailList"] == {
        "DiscussionMail": [
            {"DiscussionId": 1, "MailAddress": None, "OperationStatus": "UnknownError"},
            {"DiscussionId": 2, "MailAddress": None, "OperationStatus": "UnknownError"},
        ]
    }
