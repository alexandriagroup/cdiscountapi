...
def test_OfferPackage_with_invalid_key(valid_offer_for_package):
    ...
    # or optional
    pytest.raises(
        ValidationError, OfferPackage, {"OfferCollection": [invalid_offer_for_package]}
    )
...
def test_ProductPackage(valid_product_for_package):
    package = ProductPackage({"Products": [valid_product_for_package]})
    assert len(package.data) == 1
...
def test_ProductPackage_with_invalid_key(valid_product_for_package):
    ...
    # or optional
    pytest.raises(
        ValidationError, ProductPackage, {"Products": [invalid_product_for_package]}
    )
