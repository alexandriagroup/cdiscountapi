...
def test_last_request():
    ...
    assert (
        '<ns0:GetSellerIndicators xmlns:ns0="http://www.cdiscount.com">'
        in api.last_request
    )
    assert api.last_request.endswith("</soap-env:Envelope>\n")
...
def test_last_response():
    ...
    assert (
        ...
        in api.last_response
    )
    assert api.last_response.endswith("</s:Envelope>\n")
